...
<?php
    ...
    $sql_data = "SELECT
                ...
                WHERE
                	a.kode_skpd LIKE '" . $_SESSION["simpeg_kode_skpd"] . "%' AND
					a.status = '2'
                ORDER BY
                	a.id_usulan DESC";
...
<script type="text/javascript">
    ...
    function edit(id_usulan){
        document.location.href = "?mod=pg_daftar_usulan_edit&id_usulan=" + id_usulan;
    }
    
    function hapus(id_usulan){
        jConfirm("Anda yakin akan menghapus data usulan ini?", "PERHATIAN", function(r){
            if(r){
                document.location.href = "php/pg/pg_daftar_usulan_hapus.php?id_usulan=" + id_usulan;
            }
        });
    }
    ...
    function proses_ke_bkd(id_usulan){
        jConfirm("Anda yakin akan memproses data usulan ini ke BKD?", "PERHATIAN", function(r){
            if(r){
                document.location.href = "php/pg/pg_daftar_usulan_proses_bkd.php?id_usulan=" + id_usulan;
            }
        });
    }
    
    function daftar_diusulkan(id_usulan){
        document.location.href = "?mod=pg_daftar_usulan_diusulkan&id_usulan=" + id_usulan;
    }
	
	function acc(id_usulan){
		 document.location.href = "?mod=supervisi_pegawai_usulan_pg&id_usulan=" + id_usulan;
	}
...
<div class="panelcontainer" style="width: 100%;">
    <h3 style="text-align: left;">DAFTAR SURAT USULAN PENCANTUMAN GELAR</h3>
	<?php 
		if(isset($_GET['code']) && isset($_GET['nomor'])){
			if($_GET['code'] == 1){	
				...
				echo "Data usulan pencantuman gelar dengan No.Usulan : <b>".$_GET['nomor']." </b> diterima.</center>";
				echo "</div>";
			}else if($_GET['code'] == 2){
				...
				echo "Data usulan pencantuman gelar dengan No.Usulan : <b>".$_GET['nomor']." </b> ditolak.</center>";
				echo "</div>";
			}
		}
	?>	
    <div class="bodypanel">
    <button type="button" class="btn btn-success" onclick="document.location.href='?mod=pg_daftar_usulan_tambah';"><span class='glyphicon glyphicon-plus'></span>&nbsp;&nbsp;Tambah Surat Usulan</button>
        ...
        <script type="text/javascript">
            $.each(data, function(i, item){
                ...
                text += "<tr>";
                    ...
                    text += "<td style='text-align: center;'><button type='button' title='Tambah Pegawai Yang Diusulkan' class='btn btn-sm btn-success' style='width: 100%;' onclick='daftar_diusulkan(" + item.id_usulan + ");'><span class='glyphicon glyphicon-plus'></span></button></td>";
