...
<?php
    ...
    $sql_daftar = "SELECT
                    ...
                    FROM
                    	...
                    	INNER JOIN tbl_usulan_satya_lancana_detail i ON (a.id_pegawai = i.id_pegawai AND i.id_sk = '" . $_GET["id_usulan"] . "' AND i.status = 2)
                    ORDER BY
...
<script type="text/javascript">
    ...
    function lanjut(id_detail_satya_lencana){
        jConfirm("Anda yakin akan memproses data satya lancana pegawai ini?", "PERHATIAN", function(r){
            if(r){
                document.location.href="?mod=sl_daftar_sk_diproses_lanjut&id_detail_satya_lancana=" + id_detail_satya_lencana;
            }
        });
    }
        ...
        <script type="text/javascript">
            $.each(daftar, function(i, item){
                ...
                text += "<tr>";
                    ...
                    text += "<td style='text-align: center;'><button type='button' title='Proses Data' class='btn btn-sm btn-success' style='width: 100%;' onclick='lanjut(" + item.id_detail_satya_lencana + ");'><span class='glyphicon glyphicon-cog'></span></button></td>";
                text += "</tr>";
                document.write(text);
            });
        </script>
