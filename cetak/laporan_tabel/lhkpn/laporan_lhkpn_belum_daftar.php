...
<?php
    $whr = "";
     ...
     $sql = "SELECT
                    ...
                    CASE
                		WHEN (SELECT COUNT(*) FROM tbl_lhkpn WHERE id_pegawai = a.id_pegawai) = 0 THEN 'A'
                		ELSE 'B'
                	END AS jenis_form
                FROM
                	tbl_pegawai a
                    ...
                    INNER JOIN tbl_lhkpn_wajib i ON (a.id_pegawai = i.id_pegawai AND i.tahun='" . $_POST["tahun"] . "')
                    LEFT JOIN tbl_lhkpn j ON (i.id_pegawai = j.id_pegawai AND YEAR(j.tgl_lapor)='" . $_POST["tahun"] . "')
                WHERE
                	(a.id_status_kepegawaian = 1 OR a.id_status_kepegawaian = 4 OR a.id_status_kepegawaian = 3)
                    AND j.id_pegawai IS NULL
                    " . $whr . "
                GROUP BY
	                a.id_pegawai
                ORDER BY
...
<?php
    ...
    $html .= "<thead>";
        $html .= "<tr>";
            ...
            $html .= "<th width='12%'>JENIS KELAMIN</th>";
            $html .= "<th>SKPD</th>";
            $html .= "<th width='12%'>JABATAN</th>";
            $html .= "<th width='12%'>PANGKAT</th>";
            $html .= "<th width='7%'>JENIS FORM</th>";
        $html .= "</tr>";
    $html .= "</thead>";
    $html .= "<tbody>";
    $no = 0;
    while($ds = mysql_fetch_array($res)){
        ...
        $html .= "<tr>";
            ...
            $html .= "<td align='center'>" . $ds["jenis_form"] . "</td>";
        $html .= "</tr>";
    }
    $html .= "</tbody>";
    $html .= "</table>";
