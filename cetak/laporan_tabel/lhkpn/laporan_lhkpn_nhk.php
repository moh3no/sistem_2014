...
<?php
    $whr = "";
     ...
     $sql = "SELECT
                    ...
                    CASE
                		WHEN i.jenis_form = 1 THEN 'A'
                		WHEN i.jenis_form = 2 THEN 'B'
                	END AS jenis_form,
                    i.no_nhk
                FROM
                	tbl_pegawai a
                    ...
                    LEFT JOIN tbl_lhkpn i ON (a.id_pegawai = i.id_pegawai AND YEAR(i.tgl_lapor)='" . $_POST["tahun"] . "' AND (i.no_nhk<>'' AND i.no_nhk IS NOT NULL))
                WHERE
...
<?php
    ...
    while($ds = mysql_fetch_array($res)){
        ...
        $html .= "<tr>";
            ...
            $html .= "<td align='center'>" . $ds["no_nhk"] . "</td>";
        $html .= "</tr>";
    }
    $html .= "</tbody>";
    $html .= "</table>";
